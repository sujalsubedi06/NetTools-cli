...
from nettools.core.output import to_json, write_json
from nettools.scanner import (
    TCPScanner,
    get_common_ports,
    parse_port_range,
    resolve_target,
...
def scan(
    ...
    try:
        selected_ports = parse_port_range(ports) if ports else get_common_ports()

    except ValueError as exc:
        console.print(f"[red]Invalid ports:[/red] {exc}")
...
[cyan]Timeout:[/cyan] {timeout}s
    ...
    for result in results:
        table.add_row(
            str(result.port),
            ("[green]OPEN[/green]" if result.is_open else "[red]CLOSED[/red]"),
            result.service,
    ...
    open_ports = [result for result in results if result.is_open]
